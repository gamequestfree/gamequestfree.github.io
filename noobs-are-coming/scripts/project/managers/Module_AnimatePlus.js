...
const accelerate = ({ style }, keyframes) => (style.willChange = keyframes ? keyframes.map(({ property }) => property).join() : "auto")
...
const getDeviation = (blur, { easing }, curve) => {
	...
	const deviation = (() => {
		...
		if (easing.startsWith("in-out")) return (curve < 0.5 ? progress : out) * 2
...
const easings = {
	...
	"in-out-cubic": (progress) => ((progress *= 2) < 1 ? 0.5 * progress ** 3 : 0.5 * ((progress -= 2) * progress ** 2 + 2)),
	"in-out-quartic": (progress) => ((progress *= 2) < 1 ? 0.5 * progress ** 4 : -0.5 * ((progress -= 2) * progress ** 3 - 2)),
	"in-out-quintic": (progress) => ((progress *= 2) < 1 ? 0.5 * progress ** 5 : 0.5 * ((progress -= 2) * progress ** 4 + 2)),
	"in-out-exponential": (progress) => ((progress *= 2) < 1 ? 0.5 * 1024 ** (progress - 1) : 0.5 * (-(2 ** (-10 * (progress - 1))) + 2)),
	"in-out-circular": (progress) =>
		(progress *= 2) < 1 ? -0.5 * (Math.sqrt(1 - progress ** 2) - 1) : 0.5 * (Math.sqrt(1 - (progress -= 2) * progress) + 1),
	"in-out-elastic": (progress, amplitude, period) => {
		const strength = Math.max(amplitude, 1)
		const offset = getOffset(strength, period)
		return (progress *= 2) < 1
			? -0.5 * (strength * 2 ** (10 * (progress -= 1)) * Math.sin(((progress - offset) * pi2) / period))
			: strength * 2 ** (-10 * (progress -= 1)) * Math.sin(((progress - offset) * pi2) / period) * 0.5 + 1
	}
}

const decomposeEasing = (easing) => {
	if (typeof easing === "function") {
		return { easingFunction: easing }
	}
	const [easingName, amplitude = 1, period = 0.4] = easing.trim().split(" ")
	return { easingName, amplitude, period }
}

const ease = (easingObject, progress) => {
	if (easingObject.easingFunction) {
		return easingObject.easingFunction(progress)
	}
	const { easingName, amplitude, period } = easingObject
	const easingFunction = easings[easingName]
	if (!easingFunction) {
		console.error(`Unknown easing function: ${easingName}`)
		return progress
	}
	return easingFunction(progress, amplitude, period)
}
...
const createAnimationKeyframes = (keyframes, index) =>
	Object.entries(keyframes).map(([property, values]) => addPropertyKeyframes(property, computeValue(values, index)))
...
const reverseKeyframes = (keyframes) => keyframes.forEach(({ numbers }) => numbers.reverse())
...
const getProgress = ({ elapsed, duration }) => (duration > 0 ? Math.min(elapsed / duration, 1) : 1)
...
const tick = (now) => {
	...
	all.forEach((object) => {
		...
		const { element, keyframes, loop, optimize, direction, change, easing, duration, gaussian, end, options } = object
