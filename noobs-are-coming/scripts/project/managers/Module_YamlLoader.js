...
function composeNode(state, parentIndent, nodeContext, allowToSeek, allowCompact) {
	...
	if (indentStatus === 1 || CONTEXT_BLOCK_OUT === nodeContext) {
		...
		if (indentStatus === 1) {
			if (
				(allowBlockCollections && (readBlockSequence(state, blockIndent) || readBlockMapping(state, blockIndent, flowIndent))) ||
...
function writeScalar(state, string, level, iskey, inblock) {
	state.dump = (function () {
		...
		switch (
			chooseScalarStyle(string, singleLineOnly, state.indent, lineWidth, testAmbiguity, state.quotingType, state.forceQuotes && !iskey, inblock)
		) {
			case STYLE_PLAIN:
				return string
			case STYLE_SINGLE:
